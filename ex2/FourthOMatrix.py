#Fourth Order RK Damped Harmonic
...
###### Manually implementing Runge Kutta 4 Algorithm ######
for i in range(len(t)-1):
    ...
    k4 = A.dot(q3)+B
    qstar[:,i+1]= (qstar[:,i].reshape(3,1)+(k1+2*k2+2*k3+k4)*(h)/6).reshape(1,3)

#### Plot this.

plt.plot(t,qstar[0,:],color='blue', marker='+', label='$4^{th}$ order Runge Kutta')
